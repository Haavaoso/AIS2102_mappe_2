...
class LivePlotter(QMainWindow):
    def __init__(self, data, lock):
        ...
        self.axisTitles = ["Degrees", "Degrees", "RPM", "mA"]
        ...
        self.update_interval = 33  # Update interval in milliseconds
    ...
    def plotGraph1(self):
        current = self.plot_data[0]
        target = self.plot_data[1]
        times = self.plot_data[6]

        self.plots[0].plot(
            times, current, name="Actual", pen=pg.mkPen(color="m", width=1)
        )
        self.plots[0].plot(
            times, target, name="Target", pen=pg.mkPen(color="w", width=1)
        )
        self.plots[0].addLegend(offset=(-10, -140))

    def plotGraph2(self):
        current = self.plot_data[2]
        target = self.plot_data[3]
        times = self.plot_data[6]
        self.plots[1].plot(
            times, current, name="Actual", pen=pg.mkPen(color="m", width=1)
        )
    ...
    def plotGraph3(self):
        times = self.plot_data[6]
        self.plots[2].plot(
            times, self.plot_data[4], name="RPM", pen=pg.mkPen(color="m", width=1)
        )

    def plotGraph4(self):
        times = self.plot_data[6]
        self.plots[3].plot(
            times, self.plot_data[5], name="Voltage", pen=pg.mkPen(color="m", width=1)
        )

    def update_plot(self):
        # Update input
        with self.lock:
            self.plot_data[7] = self.packet
